...
const AdminHosts = ({result}) => {
    ...
    const handleDeleteSelectedItems = async (id) => {
        ...
        // setRows(updatedRows);
        let response = await fetch(`http://127.0.0.1:8000/auth/delete_host/${id}`, {
            method: 'DELETE'
        })
            ...
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
                ...
                <TableRow>
                    <TableCell 
                        ...
                        onChange={(event) => {
                            if (event.target.checked) {
                                setSelectedItems(result.map((row) => row.host_id))
                            }
                            else {
                                setSelectedItems([])
                            }
                ...
                {result.map((row, index) => (
                    ...
                    <TableCell component="th" scope="row">
                        <Checkbox 
                            checked={selectedItems.includes(row.host_id)}
                            onChange={(event) => handleCheckboxClick(event, row.host_id)}
                            /> 
                            {row.host_id}
                    </TableCell>
                    <TableCell align="right">{row.username}</TableCell>
                    <TableCell align="right">{row.password.slice(0, 15)}</TableCell>
                    ...
                    <Tooltip>
                        <Button
                            variant="contained"
                            color="primary"
                            onClick={() => handleDeleteSelectedItems(row.host_id)}
                            disabled={!selectedItems.includes(row.host_id)}
                        >
                            <DeleteIcon />
                        </Button>
                    </Tooltip>
